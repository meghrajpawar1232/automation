...
describe("Verify every View of PACS functionality for Reading Physician Role", () => {
  let loginData;
  before(() => {
  ...
  it("Verify user should be able to CREATE new view", () => {
    ...
    cy.loginUser(
      loginData.readPhysicianRole.username,
      loginData.readPhysicianRole.password,
      loginData.readPhysicianRole.mfaSecret
    );
    ...
    cy.get('[aria-label="Open Filter Menu"]>span').eq(9).click(); //filter menu icon
  ...
  it("Verify user should be able to Assign a view to any role and validate after assigning", () => {
    ...
    cy.get('[class="MuiBox-root css-5f1i90"]>div>h5')
      ...
      cy.get('[name="docName"][placeholder="Search"]').type('Admin').wait(2000); //for searching
      cy.get('input.PrivateSwitchBase-input').first().click(); //for selecting the checkbox
