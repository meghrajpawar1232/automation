...
describe("Verify Login Functionality for Front Desk Role", () => {
  beforeEach(() => {
  ...
  it("Login with valid username and password", () => {
    login.getUserId().type(loginData.frontDeskRole.username);
    login.getPassword().type(loginData.frontDeskRole.password);
    ...
    var token = otplib.authenticator.generate(loginData.frontDeskRole.mfaSecret);
  ...
  it("Login with invalid username and valid password", () => {
    login.getUserId().type(loginData.frontDeskRole.invalidusername, { delay: 0 });
    login.getPassword().type(loginData.frontDeskRole.password, { delay: 0 });
    login.getTermsAndCondition().click({force:true});
    login.getLoginButton().click({force:true});
  ...
  it("Login with valid username and invalid password", () => {
    login.getUserId().type(loginData.frontDeskRole.username, { delay: 0 });
    login.getPassword().type(loginData.frontDeskRole.invalidpassword, { delay: 0 });
  ...
  it.skip("Verify all the available column filter should work properly (Pin, resize, autosize, reset, set/select, preview, etc)", () => {
    cy.visit(Cypress.config("baseUrl"));
    cy.loginUser(loginData.frontDeskRole.username, loginData.frontDeskRole.password, loginData.frontDeskRole.mfaSecret);
    cy.get('[class="Toastify__toast-body"]>div')
      .contains("Signed in Successfully")
      .should("be.visible");
