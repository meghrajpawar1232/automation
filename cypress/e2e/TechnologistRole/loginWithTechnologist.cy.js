...
describe("Verify Login Functionality for Technologist Role", () => {
  beforeEach(() => {
  ...
  it("Login with valid username and password", () => {
    login.getUserId().type(loginData.technologistRole.username);
    login.getPassword().type(loginData.technologistRole.password);
    ...
    var token = otplib.authenticator.generate(loginData.technologistRole.mfaSecret);
  ...
  it("Login with invalid username and valid password", () => {
    login.getUserId().type(loginData.technologistRole.invalidusername, { delay: 0 });
    login.getPassword().type(loginData.technologistRole.password, { delay: 0 });
    login.getTermsAndCondition().click({force:true});
    login.getLoginButton().click({force:true});
  ...
  it("Login with valid username and invalid password", () => {
    login.getUserId().type(loginData.technologistRole.username, { delay: 0 });
    login.getPassword().type(loginData.technologistRole.invalidpassword, { delay: 0 });
  ...
  it.skip("Verify all the available column filter should work properly (Pin, resize, autosize, reset, set/select, preview, etc)", () => {
    cy.visit(Cypress.config("baseUrl"));
    cy.loginUser(loginData.technologistRole.username, loginData.technologistRole.password, loginData.technologistRole.mfaSecret);
    cy.get('[class="Toastify__toast-body"]>div')
      .contains("Signed in Successfully")
      .should("be.visible");
