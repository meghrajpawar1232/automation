...
describe("Verify Final report functionality for Admin Role", () => {
  let loginData;
  before(() => {
  ...
  it("Verify that the Report is saved as 'Final' when information is entered", () => {
 ...
 cy.wait(2000);
  ...
  //Validate the report as final 
  cy.get('[title="FINAL"]').scrollIntoView().should('be.visible').click();
  cy.get('[class="Toastify__toast-body"]>div')
    .contains("Report Updated")
    .should("be.visible");
  cy.wait(5000);
...
it("Verify that the Report is saved as 'Final', the 'Edit Final' text should be visible with a timer", () => {
    cy.visit(Cypress.config("baseUrl"));
  cy.loginUser(loginData.adminRole.username, loginData.adminRole.password, loginData.adminRole.mfaSecret);
  cy.get('[class="Toastify__toast-body"]>div')
    .contains("Signed in Successfully")
 ...
 cy.wait(2000);
  ...
  //validate the "Edit final" tab
  cy.get('#edit-final').contains('Edit final').should("be.visible");
  cy.wait(4000);
    })
})

it("Verify that the Report is saved as 'Final', a 'Declaration note' should be visible", () => {
    cy.visit(Cypress.config("baseUrl"));
  cy.loginUser(loginData.adminRole.username, loginData.adminRole.password, loginData.adminRole.mfaSecret);
  cy.get('[class="Toastify__toast-body"]>div')
    .contains("Signed in Successfully")
 ...
 cy.wait(2000);
  ...
  //validate the "Declaration" Note 
  cy.get('[class*="rightPortion_footerInfo"]>p')
    .contains("By clicking Save Preliminary/Final, I certify that I have reviewed the data values and other information in this report and confirm that they are clinically correct.")
    .should("be.visible");

})
})

it("Verify that the Report is saved as 'Final', the 'Next' button should be visible", () => {
    cy.visit(Cypress.config("baseUrl"));
  cy.loginUser(loginData.adminRole.username, loginData.adminRole.password, loginData.adminRole.mfaSecret);
  cy.get('[class="Toastify__toast-body"]>div')
    .contains("Signed in Successfully")
 ...
 cy.wait(2000);
  ...
  cy.get('[class*="rightPortion_footerActions"]>div>svg')
    .contains("Next")
    .should("be.visible");
