...
describe("Verify template functionality for Admin Role", () => {
  let loginData;
  before(() => {
  ...
  it("Verify that the physician can search and select the required template.", () => {
    ...
    // //Validate the draft, prelim and final section
    // cy.get('[title="DRAFT"]').scrollIntoView().should('be.visible').click();
    // cy.get('[class="Toastify__toast-body"]>div')
    //   .contains("Report Updated")
    //   .should("be.visible");
    // cy.wait(5000);
    // cy.get('[title="PRELIMINARY"]').scrollIntoView().should('be.visible').click();
    // cy.get('[class="Toastify__toast-body"]>div')
    //   .contains("Report Updated")
    //   .should("be.visible");
    // cy.wait(5000);
    ...
    // //Validate start typing in a textbox
    // cy.get('[data-placeholder="Press record button to start Dictating..."]').first().type('Check the text typing in the impression section line change the new line is here');
    // //validating Impression section
    // cy.get('[data-testid="MenuIcon"').should('be.visible').click();
    // cy.wait(2000);
    // cy.get('[id="criticalFinding_select"]')
    //   .contains('Impression section')
    //   .should('be.visible');
...
it("Verify Dictation module functionality in PACS", () => {
  cy.visit(Cypress.config("baseUrl"));
  cy.loginUser(loginData.adminRole.username, loginData.adminRole.password, loginData.adminRole.mfaSecret);
  cy.get('[class="Toastify__toast-body"]>div')
    .contains("Signed in Successfully")
