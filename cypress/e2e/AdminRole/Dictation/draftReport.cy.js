...
describe("Verify Draft report functionality for Admin Role", () => {
  let loginData;
  before(() => {
  ...
  it("Verify that the Report is saved as 'Draft' when initial information is entered", () => {
 ...
 cy.get('[ref="eCheckbox"]').contains("(Select All)").click({force:true});
 cy.wait(5000);
 cy.get('[aria-label="Search filter values"]').type("COMPLETED").wait(2000);
 cy.get('[ref="eCheckbox"]').contains("COMPLETED").click({force:true});
 cy.wait(2000);
  ...
  //Remove the already present draft report on Study instance page
...
it("Verify that another user can take over a draft report created by a physician.", () => {
  cy.visit(Cypress.config("baseUrl"));
cy.loginUser(loginData.adminRole.username, loginData.adminRole.password, loginData.adminRole.mfaSecret);
cy.get('[class="Toastify__toast-body"]>div')
  .contains("Signed in Successfully")
...
cy.get('[ref="eCheckbox"]').contains("(Select All)").click({force:true});
cy.wait(5000);
cy.get('[aria-label="Search filter values"]').type("ASSIGNED").wait(2000);
cy.get('[ref="eCheckbox"]').contains("ASSIGNED").first().click({force:true});
cy.wait(2000);
...
cy.pause()
//validate the draft takeover popup
