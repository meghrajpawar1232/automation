...
import UnscheduledOrderPage from "../../../pages/unScheduledOrderPage";

const adminpage = new AdminPage();
const docManager = new DocumentManagerPage();
const unscheduledOrder = new UnscheduledOrderPage();
...
describe("Verify Creating Order Functionality in Document Manager and validating in the Unscheduled orders page for Admin Role", () => {
  beforeEach(() => {
  ...
  it("Verify Orders can be created for newly added patient with assigning document and should be visible in unscheduled orders page", () => {
    ...
    cy.get('[class*="doclist_list"]>div').then(($list) => {
      if (!Cypress.dom.isVisible($list)) {
        ...
        cy.get('[aria-labelledby="address-country-code-label"]>li')
          ...
          .click()
          .wait(2000);
        docManager.phoneNumberTxtField().clear().type(phoneNum, { delay: 0 });
        //save the details
    ...
    //Validating the created order in the unscheduled order
    unscheduledOrder.clickUnscheduledOrderIcon();
    cy.wait(12000);
    
    //filter the appointment status "Order created"
    cy.get('[aria-label="Open Filter Menu"]>span').last().click();
    cy.get('[ref="eCheckbox"]').contains('(Select All)').click();
    cy.wait(4000);
    cy.get('[ref="eCheckbox"]').contains('Order Created').click();
    cy.wait(10000);

    //filter the newly created patient name
