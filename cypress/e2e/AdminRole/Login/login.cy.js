...
describe("Verify Login Functionality for Admin Role", () => {
  beforeEach(() => {
  ...
  it("Login with valid username and password", () => {
    login.getUserId().type(loginData.adminRole.username);
    login.getPassword().type(loginData.adminRole.password);
    ...
    var token = otplib.authenticator.generate(loginData.adminRole.mfaSecret);
  ...
  it("Login with invalid username and valid password", () => {
    login.getUserId().type(loginData.adminRole.invalidusername, { delay: 0 });
    login.getPassword().type(loginData.adminRole.password, { delay: 0 });
    login.getTermsAndCondition().click({force:true});
    login.getLoginButton().click({force:true});
  ...
  it("Login with valid username and invalid password", () => {
    login.getUserId().type(loginData.adminRole.username, { delay: 0 });
    login.getPassword().type(loginData.adminRole.invalidpassword, { delay: 0 });
