...
describe("Verify Login Functionality for Rad Assistant role", () => {
  beforeEach(() => {
  ...
  it("Login with valid username and password", () => {
    login.getUserId().type(loginData.radAssisstantRole.username);
    login.getPassword().type(loginData.radAssisstantRole.password);
    ...
    var token = otplib.authenticator.generate(loginData.radAssisstantRole.mfaSecret);
  ...
  it("Login with invalid username and valid password", () => {
    login.getUserId().type(loginData.radAssisstantRole.invalidusername, { delay: 0 });
    login.getPassword().type(loginData.radAssisstantRole.password, { delay: 0 });
    login.getTermsAndCondition().click({force:true});
    login.getLoginButton().click({force:true});
  ...
  it("Login with valid username and invalid password", () => {
    login.getUserId().type(loginData.radAssisstantRole.username, { delay: 0 });
    login.getPassword().type(loginData.radAssisstantRole.invalidpassword, { delay: 0 });
